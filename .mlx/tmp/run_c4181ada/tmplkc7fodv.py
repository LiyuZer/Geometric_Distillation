...
SETUP = {"final_val_accuracy": 1.71, "torch_seed": 42, "seed": 42}
...
def train_factorized(rank):
    torch_seed = SETUP.get('torch_seed')
    random_seed = SETUP.get('seed')
    ...
    for epoch in range(num_epochs):
        ...
        for images, labels in tqdm(train_loader, desc=f'Epoch {epoch + 1}/{num_epochs}'):
            ...
            for orig_out, fact_out in zip(mlp_outputs, factorized_mlp_outputs):
                ...
                print(fact_out.shape, orig_out.shape)
...
def wrapper_train_factorized():
    try:
        result = train_factorized(rank=32)
